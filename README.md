...
- `maoyan_format`：猫眼返回格式，`json|text|image`（默认 `image`）
...
## 使用说明
- 直接发送中文指令即可，不需要携带参数
- 若接口返回图片，将以图片形式发送；返回文本或 json 时按各平台配置输出